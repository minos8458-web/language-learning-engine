...
> ⚠️ **이 절은 확정이 아니다.** `LEARNING_PROTOCOL.md`(Missing Input Document)가 확보되면 Learning State Assessment·목표 결정 로직 전체와 대조해 개정하거나, `start_session`의 목표 결정 로직과 단일 함수로 통합해야 한다. 지금은 `request_practice`를 개별적으로 막고 있는 좁은 범위(동일 언어에 `INTRODUCED`/`STUDYING` 노드가 여럿인 경우의 선택)만 해소한다.
