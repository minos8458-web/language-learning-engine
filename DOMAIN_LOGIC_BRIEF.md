...
- Content의 `type_specific_metadata.error_attributed_node_id`(**AC-011, 2026-07-08 Resolved로 키 이름 확정** — TEXT, nullable, 단일 `grammar_node_id`, 채점 가능한 content_type에 선택적 적용·QUIZ 한정 아님)에 오류 세그먼트를 특정 `grammar_node_id`로 귀속하는 진단 정보가 있고, 그 노드가 `primary_node_id`와 **다르면** → `TRANSFER`, 귀속된 그 노드를 대상으로 5.2 진행.
- 이 키가 부재하거나 `null`이거나 `primary_node_id`와 같은 값이면 → `SELF`(선행 탐색 생략, 현재 노드 반복 연습으로 처리). 셋 다 동일하게 SELF로 처리한다(AC-011).
- **기본값은 항상 SELF다** — 정밀 진단이 없는 한 TRANSFER로 함부로 판정해 불필요한 선행 탐색을 트리거하지 않는다. 복수 오류 세그먼트 처리는 AC-011 결정 범위 밖(단수 대상 전제, 필요 시 별도 AC 필요).
- Content 자신의 `grammar_node_ids` 밖 노드로 귀속하는 것도 허용된다(AC-011 — §5.1이 제약을 두지 않음). 실존 노드인지는 저작·적재 시점에 검증한다.
...
## 8. Generation 대상 노드 결정 & AI Generation 게이팅

### 8.1 `target_node_id` 선택 규칙 (AC-009, 2026-07-08 Provisional — `LEARNING_PROTOCOL.md` 확보 후 재검토 예정)

AC-005가 사다리 3단계(사전 제작 콘텐츠 대체) 진입 시 `target_node_id`는 Learning Flow Engine이 결정한다고 확정했으나, 후보가 여럿일 때(동일 언어에 `INTRODUCED`/`STUDYING` 상태 노드가 여러 개인 경우) 어떤 것을 고를지는 이 절이 정의한다.

1. `target_concept_id`가 주어지면 그 Concept 노드로 후보를 좁힌다.
2. **주 규칙**: `progress.state ∈ {INTRODUCED, STUDYING}`인 노드 중 `explicit_study_event_at DESC`(가장 최근 명시적 학습 시작)로 선택. `updated_at`은 다른 노드의 시도 처리 등 무관한 갱신에도 변하는 범용 컬럼이라 채택하지 않았다 — `explicit_study_event_at`은 "이 노드를 명시적으로 배우기 시작한 시점"만 순수하게 반영하는 전용 컬럼이다(`DATA_PERSISTENCE_BRIEF.md` §3.6).
...
**Provisional 표시 이유**: AC-001·005·008·011과 달리 문서 어디에도 아직 선언된 적 없는 신규 정책 판단이었다(계약 누락 보완이 아님). `LEARNING_PROTOCOL.md` 확보 후 이 규칙이 그 문서의 실제 내용과 다르면 개정 대상이며, 그때는 `start_session`(API_CONTRACT §10.5)의 목표 결정 로직과 단일 함수로 통합하는 것을 원칙으로 남겨둔다.

### 8.2 AI Generation 게이팅 & 4단계 사다리
...
| 1.1 | 2026-07-08 | AC-009 Provisional 반영 — §8.1 `target_node_id` 선택 규칙 신설, 기존 §8은 §8.2로 이동 |
| 1.2 | 2026-07-08 | AC-011 Resolved 반영 — §5.1 SELF/TRANSFER 분류에 `error_attributed_node_id` 키 이름 확정 |
| — | 2026-07-11 | **Contract Reconciliation 패치** — 코드베이스 유실 후 재구현 착수 전, GitHub 본문이 v1.0에 머물러 있던 것을 위 AC-009/011 Resolved·Provisional 결정 기준으로 일괄 반영. 새로운 설계 결정 없음. 근거: `REBUILD_CONTRACT_RECONCILIATION.md` |
