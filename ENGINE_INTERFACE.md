...
| AI Generation Engine | O (Concept 존재성, node metadata·label·Concept membership 조회 및 선택 결과 검증. 첫 planning의 ALTERNATIVE 확장은 하지 않고 stage 5와 함께 2차 검토) | O (Practicing 이상 필터·최근 시도 조합 조회) |
...
| **3. 입력 데이터** | canonical positional `generateProblem(pool, userId, language, targetConceptId, targetNodeId)`. 마지막 두 인자만 선택이다. `target_concept_id`는 1~2단계용이고, **`target_node_id`는 3단계 PRE_MADE fallback 전용 — AC-005, 2026-07-08 Resolved. Learning Flow Engine이 결정해 전달, 이 Engine은 3단계에서 Content Engine으로 그대로 릴레이만 함** |
...
| **8. 향후 구현 시 주의사항** | "지금 몇 단계인가"와 retry/강등을 판단하는 정책 로직은 **이 Engine에만** 있어야 한다. Provider technical failure는 1회 재시도 후 강등, constraint violation은 총 최대 3회 생성 후 강등, candidate 부족은 즉시 강등한다. 반면 save 실패는 즉시 throw하고 PRE_MADE read technical failure는 AI 설정과 독립적으로 1회 재시도 후 throw한다. PRE_MADE 0건만 정상 empty로 4단계 강등, 1건은 반환, 2건 이상은 임의 선택·강등 없이 sanitized invariant error로 throw한다 |

**AC-018 construction과 lazy validation**: `createGenerationEngine({aiGenerationEngine,contentEngine})`는 `{generateProblem}`을 반환한다. 두 dependency는 각각 non-null object여야 하고 AI dependency의 `selectGenerationCandidates`·`generateCombination`·`generateSingleNode`, Content dependency의 `getRecentGeneratedContent`·`saveGeneratedContent`·`getContent`가 모두 function이어야 한다. 하나라도 빠지면 composition 시점에 동기 `TypeError`를 throw한다. closure는 이 injected dependency만 사용하며 Graph/Progress 직접 import, direct SQL, module mutable singleton, setter·test-only branch를 두지 않는다. Factory는 기존 positional API를 변경하지 않으며 엔진 API 수에도 포함하지 않는다.

`targetConceptId`는 최초 planning 호출이 존재성을 검증하고, `targetNodeId`는 사다리 3단계에 실제 진입한 경우에만 `getContent`가 검증한다. 1·2단계 성공 시 미사용 `targetNodeId`를 조회하지 않는다. Generation Engine은 AC-005의 caller-side `targetNodeId`/language 책임을 유지하고 Graph Engine을 직접 호출하지 않는다.
...
| 항목 | 내용 |
|---|---|
| **1. 책임** | 난이도 사다리 **1~2단계만** 수행. `selectGenerationCandidates`가 GRAMMAR_GRAPH §6.1의 후보 계획 1~3단계(eligible pool, mode/Concept 필터, 최근 exact 조합 tie-break)를 실행해 정렬된 node ID 또는 null을 반환한다. `targetConceptId` 지정 시 첫 단계에서 `getConceptCategories` 단건 호출로 존재성만 검증하고 category 값은 정책에 사용하지 않으며, Concept 포함은 `getNodeLanguageAndConcepts`의 `concept_ids`로 판정한다. 첫 planning에서는 ALTERNATIVE 관계를 확장하지 않는다. `generateCombination`/`generateSingleNode`는 그 preselected ID와 최근 생성 Content 및 `getNodeLabels`의 canonical label을 받아 stage 4 provider prompt를 구성하고 structured output `{generated_text, answer_key, self_reported_node_ids}`를 exact schema로 검증해 Layer 2 exact `{candidate: object\|null}`로 변환 |
...
| **5. 호출 가능한 하위 Engine** | Graph Engine(읽기: `get_concept_categories`, `get_node_language_and_concepts`, `get_node_labels`), Progress Engine(읽기: `get_eligible_nodes`, `get_recent_attempted_combinations`) |
...
| **8. 향후 구현 시 주의사항** | §6.1 필터 우선순위 순서는 절대 바뀌지 않아야 한다. planning tie-break는 최근 20개 시도에서 동일 조합의 exact-match 횟수가 적은 후보를 우선하고, 동률이면 정렬 배열의 원소별 lexicographic 순서를 사용한다. join·JSON 등 직렬화 문자열을 비교 키로 사용하지 않는다. generation은 `get_node_language_and_concepts`로 모든 node의 존재성·중복 없음·language 일치를 다시 검증하지만 선택을 바꾸지 않는다. `self_reported_node_ids`는 교차확인 힌트일 뿐 단독 실패·feedback·재선정·출력 근거가 아니며 Layer 2 이후 노출하지 않는다. `generated_text` 검증 실패 시 질문과 `answer_key`를 함께 폐기한다. GRAMMAR_GRAPH §6.1 stage 5 품질 비교형 스코어링은 복수 실제 생성물을 비교하는 2차 실제 LLM milestone로 유보하며, 첫 Mock 구현은 이를 수행하거나 수행했다고 주장하지 않는다 |

**AC-018 provider·validator boundary**: generation request는 exact `{language,grammar_nodes:[{node_id,label}],recent_generated_texts,regeneration_feedback}`다. node/label은 planning ID ASC 순서, recent text는 query 순서의 `media_assets[0].asset_ref` 0~5개, 최초 feedback은 `[]`이고 이후에는 non-empty violation 문자열을 쓴다. raw output은 exact `{generated_text,answer_key,self_reported_node_ids}`다. 순서는 raw schema → deterministic rule → COMBINATION에서만 exact validator `{language,grammar_nodes,generated_text}`이며 validator output은 exact `{is_valid,violations}`다. rule 실패는 validator를 호출하지 않고 deterministic feedback으로 재생성한다. validator false는 violations를 변경 없이 feedback으로 사용한다. SINGLE_NODE는 rule까지만 수행한다.

Raw schema·rule·validator false는 최초 1회+재생성 최대 2회의 shared generation pool을 소비한다. generation과 validator의 timeout·network·parse/schema technical retry는 각각 동일 request로 1회이며 서로 독립이다. validator technical 재실패는 regeneration을 소비하거나 throw하지 않고 강등한다. `generateStructuredContent(request)`와 `validateGeneratedContent(request)` adapter operation은 각각 raw/validator exact object를 resolve하고, timeout·vendor 변환을 소유해 민감정보 없는 sanitized technical `Error`로 reject한다.

`createAiGenerationEngine({providerAdapter})`는 `{selectGenerationCandidates,generateCombination,generateSingleNode}`를 반환한다. adapter는 non-null object이고 두 operation이 모두 function이어야 하며 아니면 composition 시점 동기 `TypeError`다. implicit default adapter와 unconfigured adapter의 성공 반환은 금지한다. Production은 두 operation 모두 sanitized technical `Error`로 reject하는 명시적 unconfigured adapter를 주입해 fail-closed 강등하며 실제 vendor는 2차 milestone이다. 테스트는 동일 boundary에 deterministic fake를 주입한다. Factory/adapter operation은 엔진 API 수에 포함하지 않는다.
