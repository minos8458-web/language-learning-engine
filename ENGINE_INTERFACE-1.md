...
| **3. 입력 데이터** | 사용자 액션 이벤트 — 세션 시작(`start_session`), 명시적 학습 시작(`start_explicit_study`), 인출 시도 제출·결과(`submit_attempt`), 연습 문제 요청(`request_practice`), 자기보고 Confidence(`submit_self_reported_confidence`). 이 5개가 외부에 노출되는 API 전부다(API_CONTRACT.md §10.1~10.5). `start_session`은 매 세션의 첫 호출이며, Learning State Assessment·목표 결정·Session Budget(LEARNING_PROTOCOL.md §2~5)을 이 Engine이 내부적으로 수행해 `next_action` 하나로 정규화한 뒤 반환한다 — 정책 판단은 전적으로 이 Engine의 책임이며 클라이언트는 결과만 소비한다(1장 책임 원칙 재확인) |
...
| 1.6 | 2026-07-07 | `CLIENT_BRIEF.md` 작성 중 발견된 구조적 공백(세션 시작 진입점 부재)을 메우기 위해 `API_CONTRACT.md` v1.3에 추가된 `start_session`(10.5) 반영 — Learning Flow Engine §3 입력 데이터에 추가, 5개 외부 API로 갱신. 이 API가 Learning State Assessment·목표 결정·Session Budget을 내부 수행해 `next_action`으로 정규화 반환함으로써 "정책과 실행의 분리"(11장) 원칙이 클라이언트 경계에서도 유지됨을 명시 |
