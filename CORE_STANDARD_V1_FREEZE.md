...
| 1.3 | 2026-07-13 | **Freeze 이후 두 번째 공식 Core Standard Amendment 승인·처리(§5 절차 완료).** Independent Architecture Audit(AUD-003) — `GRAMMAR_SCHEMA.md` §6 Grammar Relation에 same-language invariant 추가(`from_node_id`·`to_node_id`가 참조하는 Grammar Node의 `language`가 반드시 동일, `PREREQUISITE`/`RELATED`/`CONTRAST`/`ALTERNATIVE` 4종 전부 적용). **Counterexample**: `src/engines/graphEngine.js`의 `prerequisiteSearchInternal`·`findRelatedNodes`·`validateLanguagePack`이 언어 일치를 전혀 확인하지 않아 cross-language `PREREQUISITE`(예: `GRAMMAR_VI_* → GRAMMAR_EN_*`)가 적재·탐색·validation 통과 모두 가능한 상태였음이 GitHub main 코드 직접 조회로 확인됨. **검토 대안**: traversal filter only(저장 계층에 무의미한 edge가 남아 기각) / DB trigger(도메인 로직을 Engine 계층에 두는 기존 스타일에서 벗어나 기각) / `grammar_relations.language` 중복 컬럼(불필요한 중복이라 기각) / Tier A invariant + `validate_language_pack` 정적 검증 + runtime traversal defense-in-depth 3중 방어(**채택**). **Governance 재분류 경위**: 1차 판정("필드 추가 없음이므로 Architecture Clarification")을 재심사 후 철회 — entity의 admissible state space를 좁히는 것 자체가 `CORE_STANDARD_V1_FREEZE.md` §2·§4가 보호하는 "엔터티 정의"에 해당하고, `GRAMMAR_GRAPH.md` §2의 기존 문언("언어별 서브그래프는 조회 조건으로 구성되는 view")만으로는 이 invariant가 Freeze 시점에 이미 규범적으로 강제됐다고 확정할 문헌적 근거가 불충분했기 때문에 Tier A Amendment로 재분류. **사용자 승인**: 명시적 승인 완료. **반영**: `GRAMMAR_SCHEMA.md`(v1.7)·`GRAMMAR_GRAPH.md`(v1.5)·`API_CONTRACT.md`(v1.10)·`ENGINE_INTERFACE.md`(v1.10)·`VALIDATION_LEVEL3.md`(v1.5) canonical 개정, `MIGRATION_GUIDE.md` Entry 004. DB schema/SQL migration 없음(기존 필드 조합에 논리적 invariant만 추가) |
