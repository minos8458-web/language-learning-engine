...
**AC-018 한정**: `self_reported_node_ids` mismatch 하나만으로 실패시키거나 `regeneration_feedback`을 생성하지 않는다.

### 2.3 Canonical node source

결과 `grammar_node_ids`는 provider 응답에서 파생하지 않는다. Provider 호출 전에 AI Generation Engine의 `select_generation_candidates` / `selectGenerationCandidates(pool, userId, language, generationMode, targetConceptId)`가 Graph/Progress canonical 데이터로 확정한 정렬·중복 제거 node 목록을 Layer 2 candidate에 그대로 사용한다. `targetConceptId`가 지정되면 planning 최초 단계에서 `getConceptCategories(pool,[targetConceptId])`를 1회 호출해 존재성만 판정한다. 미존재는 기존 `INVALID_ID`, 존재 시 category 값은 정책에 사용하지 않는다. Concept 포함은 `getNodeLanguageAndConcepts`의 `concept_ids`로 판정하며 첫 planning은 ALTERNATIVE를 확장하지 않는다. `generateCombination`과 `generateSingleNode`는 이 ID를 재선택·재필터·재정렬하지 않는다.

### 2.4 Provider raw exact schema 및 1차 Mock 기준

각 provider generation call의 exact request는 아래와 같다. 모든 필드가 required이고 모든 object는 `additionalProperties:false`다.
...
`grammar_nodes`는 planning ID를 ASC 정렬한 순서로 `getNodeLabels`의 canonical label을 결합하며 COMBINATION은 길이 2, SINGLE_NODE는 길이 1이다. `recent_generated_texts`는 최근 Content의 `media_assets[0].asset_ref`를 query 순서 그대로 추출한 0~5개 배열이다. `regeneration_feedback`는 non-empty/non-whitespace string 배열이고 최초 호출은 `[]`다.
...
Generation Engine은 반환 배열을 추가·삭제·재정렬하거나 필드를 다시 만들지 않고 `generateCombination(pool, language, grammarNodeIds, recentGeneratedContent)` 또는 `generateSingleNode(pool, language, grammarNodeIds, recentGeneratedContent)`에 그대로 전달한다. AI Generation Engine은 배열과 각 item의 exact `{content_id,media_assets,created_at}` 3-key shape를 검증한 뒤 위 exact request의 `recent_generated_texts`로 투영한다. 이 배열은 stage 4 prompt-only 입력이며 candidate 선택·node 검증·출력 근거가 아니다.
...
- **화이트리스트 내용**: 대상 노드(조합이면 전부)의 canonical `label`. AC-018 exact provider request에는 그 밖의 node metadata를 추가하지 않는다.
...
target Concept 지정 시 존재성 단건 조회
→ stage 1 eligible pool → stage 2 mode/Concept 필터(ALTERNATIVE 확장 없음)
...
stage 4 Layer 1 provider exact request:
{language, grammar_nodes, recent_generated_texts, regeneration_feedback}
...
Validator exact request는 다음이며 모든 필드가 required이고 모든 object는 `additionalProperties:false`다.

```
{language:string, grammar_nodes:[{node_id:string,label:string}], generated_text:string}
```

`grammar_nodes`는 generation request와 같은 ASC planning ID·canonical label 배열이다. exact response는 `{is_valid:boolean, violations:string[]}`이며 두 필드가 required이고 `additionalProperties:false`다. violation 원소는 non-empty/non-whitespace string이다. `is_valid=true` iff `violations=[]`, `is_valid=false` iff violation이 1개 이상이다.

실행 순서는 generation raw schema → Rule → Rule 통과 시에만 validator다. Rule 실패 시 validator를 호출하지 않고 deterministic rule violation 문자열을 feedback으로 사용한다. Validator false는 content constraint violation이며 `violations`를 변경 없이 regeneration feedback으로 전달한다.
...
1. **Generation provider technical failure**: timeout, network, structured-output parse failure는 동일 request와 동일 feedback으로 같은 provider 호출을 1회 즉시 재시도한다. 재시도도 실패하면 throw하지 않고 다음 ladder 단계로 강등한다.
2. **Provider schema/rule/validator content constraint violation**: raw schema violation에는 deterministic violation 문자열을, Rule 실패에는 deterministic rule violation 문자열을, validator false에는 `violations`를 변경 없이 feedback으로 반영한다. 최초 생성 1회 후 최대 2회 재생성하는 하나의 shared pool(총 generation provider call 최대 3회)을 사용한다. 전부 실패하면 다음 ladder 단계로 강등하고 내부 reason은 public payload에 노출하지 않는다.
...
Validator timeout·network·parse·response schema violation은 동일 validator request를 1회 재시도한다. 재실패하면 throw하지 않고 regeneration budget도 소비하지 않은 채 다음 ladder 단계로 강등한다. Generation technical retry와 validator technical retry는 독립이다. PRE_MADE read retry도 AI provider/validator 설정과 독립된 contract-fixed 1회다. PRE_MADE는 0건=정상 empty, 1건=반환, 2건 이상=sanitized invariant error throw이며 다건에서 임의 선택·강등하지 않는다.
...
| API | 입력/역할 | 출력·검증 |
|---|---|---|
| `select_generation_candidates` / `selectGenerationCandidates(pool,userId,language,generationMode,targetConceptId)` | Generation Engine만 호출. target Concept 존재성 단건 확인 후 stages 1~3에서 eligible pool·Concept·최근 시도 조합을 적용. 첫 planning ALTERNATIVE 확장 없음 | exact `{grammar_node_ids:string[]\|null}`; mode 1은 2개, mode 2는 1개; dedupe·사전순 정렬 |
...
### 7.1 Provider adapter와 composition boundary

Provider adapter는 `generateStructuredContent(request)`와 `validateGeneratedContent(request)` 두 operation을 제공한다. 전자는 provider raw 3-field를 파싱한 구조체, 후자는 exact validator response를 resolve하며 field-level schema 검증은 AI Generation Engine 책임이다. Adapter는 timeout·vendor/network error를 credential·prompt 원문·vendor raw payload 없는 sanitized technical `Error`로 변환한다. timeout milliseconds와 vendor SDK는 구현 HOW다.

`createAiGenerationEngine({providerAdapter})`는 `{selectGenerationCandidates,generateCombination,generateSingleNode}`를 반환한다. adapter가 non-null object가 아니거나 두 operation 중 하나가 function이 아니면 composition 시점에 동기 `TypeError`다. `createGenerationEngine({aiGenerationEngine,contentEngine})`는 `{generateProblem}`을 반환한다. AI dependency에는 `selectGenerationCandidates`·`generateCombination`·`generateSingleNode`, Content dependency에는 `getRecentGeneratedContent`·`saveGeneratedContent`·`getContent` function이 모두 있어야 하며 두 dependency는 non-null object여야 한다. 위반은 모두 composition 시점 동기 `TypeError`이고 새 public error code는 없다.

두 Engine closure는 주입 dependency만 사용한다. direct SQL, Graph/Progress direct import, module mutable singleton, setter, test-only branch, implicit default adapter를 금지한다. Production composition은 `createUnconfiguredProviderAdapter()`를 명시적으로 생성해 `createAiGenerationEngine({providerAdapter})`에 주입하고, 그 반환을 `createGenerationEngine({aiGenerationEngine,contentEngine})`에 주입한다. Unconfigured adapter의 두 operation은 credential·prompt·vendor raw payload 없는 sanitized technical `Error`로 reject해 PRE_MADE로 fail-closed 강등하며 성공을 반환해서는 안 된다. 실제 vendor 통합은 2차 milestone이고 테스트는 contract-identical deterministic fake adapter를 AI factory에 주입한 뒤 그 반환을 Generation factory에 주입한다. Factory와 adapter operation은 엔진 API 총수에 포함하지 않는다.
...
| 1.2 | 2026-07-22 | AC-018 Tier C Architecture Clarification — Concept 존재성·node label 경로, exact generation/validator request-response, deterministic feedback·shared regeneration과 독립 technical retry, provider adapter·factory composition validation, PRE_MADE cardinality 및 fail-closed production boundary를 확정. 외부 API 5·내부 27·전체 32, 비교형 stage 5와 실제 vendor 구현은 2차 milestone, 코드·DB·Tier A 불변 |
