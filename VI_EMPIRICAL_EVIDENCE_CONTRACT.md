...
| Participant deletion policy | `OWNER-APPROVED` | Privacy/Operations | 2026-08-21 owner decision: current provider-free/raw-audio-free P1 participants may request deletion of participant-linked pilot evidence at any time without providing a reason; the deletion request becomes effective at the earliest point when designated Privacy/Operations personnel receive the request and can reliably link it to the applicable pseudonymous `participant_id`, with no additional waiting period before in-scope evidence enters deletion-pending status; the request cutoff covers participant-linked current-P1 evidence generated at or before that effective point, including enrollment/assignment/session/attempt facts, learner text responses, timing, target-node evaluations, correction aggregates, participant-linked observations, participant-level derived/materialized artifacts, and any later-lawfully-existing participant-linked human-rating output tied to that evidence without authorizing human rating; while deletion is pending, in-scope evidence may not be newly used for pilot/efficacy-analysis preparation, human rating, audit sampling, export, or new derived/materialized analysis except minimum Privacy/Operations use necessary to execute and verify deletion; fulfillment removes participant-linked raw/participant-level evidence from active pilot evidence rather than merely hiding or inactivating it, while any necessary deletion/tombstone lineage must be minimum non-content lineage and must not retain learner payload, direct PII, identity↔participant mapping value, or deleted evidence payload as ordinary evidence; rebuildable/materialized artifacts that contain participant-level data or still incorporate the deleted participant's contribution must not be used after fulfillment until regenerated/recomputed excluding that contribution; deletion lineage must record applicable evidence-class disposition and policy/version without preserving deleted payload, while lineage retention duration remains governed by separately approved Evidence retention; deletion alone does not constitute withdrawal, so absent a withdrawal request the cutoff does not silently delete future evidence collected under otherwise-valid consent, whereas a combined withdrawal+deletion request applies the separately approved Withdrawal handling immediately and deletion independently without delaying withdrawal; late technical receipts demonstrably generated at or before the cutoff remain in deletion scope, and receipts whose generation relative to the cutoff cannot be reliably established must remain unavailable for ordinary analysis/use until Privacy/Operations resolves their deletion disposition; mapping may be used only as already authorized to locate, execute, verify, and close deletion and this row does not change mapping retention; this policy sets no general Evidence retention duration and authorizes no Operational support, Incident handling, Human rater access, provider/audio path, P1 activation, human-data collection, or efficacy conclusion, and materially different future provider/audio evidence classes require versioned/re-approved deletion semantics before activation |
