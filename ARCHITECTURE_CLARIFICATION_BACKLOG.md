...
**Implementation boundary**: 이번 단계는 문서 전용 patch다. `engineConfig.js`, 코드·테스트·DB schema/migration/entity, Tier A 문서를 수정하지 않는다. `request_practice` 구현·계약 확장과 Conversation Engine 내부 구현은 후속 milestone으로 분리한다. AC-017 prerequisite implementation은 **NOT STARTED**이며 IMPLEMENTED/CLOSED/PASS 또는 Validation Level 3 PASS를 선언하지 않는다.
