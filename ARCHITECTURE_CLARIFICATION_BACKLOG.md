...
**AC-014 Progress/Graph-side validation evidence(2026-07-19, additive)**: 검증 브랜치 `ac-014-read-api-validation-20260719`의 implementation commit `bfb6c42cb9f277b3bffc7ad43e76a1e24a9b96a4`에서 신규 내부 read API 4개와 실제 PostgreSQL 테스트를 구현했다. Workflow-only baseline run `29688270347`은 97/97 PASS, 21 suites였고, implementation run `29688678717`은 PostgreSQL 16.14 / Node.js 20.20.2에서 신규 28개를 포함한 125/125 PASS, 26 suites, 0 FAIL / 0 CANCELLED였다. 유효·미존재 ID 혼합 요청은 부분 결과 없이 거부하고, Progress read API는 동일 client를 사용하며 성공·오류 경로 모두 `finally`에서 release함을 실제 DB 테스트로 검증했다. 이 API들은 read-only이므로 write transaction/rollback 대상이 없으며 DB schema/migration 변경도 없다. 본 evidence는 독립 코드리뷰 전 검증 사실만 기록하며 상태는 Architecture Clarification RESOLVED / Prerequisite Implementation IN PROGRESS이고, IMPLEMENTED·CLOSED 또는 Validation Level 3 §9 PASS를 선언하지 않는다.
...
| 1.22 | 2026-07-19 | AC-014 Progress/Graph-side implementation validation evidence — validation branch implementation `bfb6c42cb9f277b3bffc7ad43e76a1e24a9b96a4`, baseline run `29688270347` 97/97 PASS·21 suites, implementation run `29688678717` 125/125 PASS·26 suites·PostgreSQL 16.14·Node.js 20.20.2를 additive 기록. 신규 read API 4개 구현 후 상태는 RESOLVED / IN PROGRESS로 유지하며 독립 코드리뷰 전 IMPLEMENTED·CLOSED·§9 PASS는 선언하지 않음 |
