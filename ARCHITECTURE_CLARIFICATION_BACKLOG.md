...
**AC-015 implementation validation evidence(2026-07-20, additive)**: 검증 브랜치 `ac-015-interleaving-validation-20260720`의 workflow-only baseline commit `0f36fc8ec932ca3423d0e0d20ffc72382133f8d8`과 run `29704547463`에서 PostgreSQL 16.14 / Node.js 20.20.2, 125/125 PASS·26 suites를 확인했다. Implementation commit `b0e368b3a0df1eaa383eec5013544ef56e2afd37`은 허용된 6개 파일(`src/config/engineConfig.js`, `src/engines/graphEngine.js`, 신규 `src/engines/interleavingEngine.js`, `tests/graphEngine.test.js`, 신규 `tests/interleavingEngine.test.js`, 이 backlog)만 변경했다. 최초 run `29704717846`은 149/150 PASS·32 suites로, production 결함이 아니라 multi-category shared-penalty를 피한 canonical 결과 `E,D,F`에 대해 테스트가 `D,E,F`를 잘못 기대한 1건 때문에 실패했다. 공개 이력을 재작성하지 않고 test-only correction commit `aea971d9bcfa5180772ab7aee69021ec636a0fa4`에서 기대값 한 줄을 정정했으며 run `29704796941`은 신규 25개를 포함해 150/150 PASS·32 suites, 0 FAIL / 0 CANCELLED였다. `getNodeLanguageAndConcepts`의 validation·중복 정규화·부분 결과 없는 INVALID_ID·mixed-language fact 반환, `sequenceNodes`의 config 기반 batch 우선 검증·language boundary·multiplicity 보존·Category/CONTRAST/lexicographic ordering·Graph-only dependency를 실제 PostgreSQL fixture로 검증했다. AC-014와 AC-015는 모두 RESOLVED / Prerequisite Implementation IN PROGRESS이며, 독립 리뷰 전 IMPLEMENTED·CLOSED 또는 Validation Level 3 §9 PASS를 선언하지 않는다.
...
| 1.24 | 2026-07-20 | AC-015 implementation validation evidence — baseline `0f36fc8` / run `29704547463` 125/125 PASS·26 suites, implementation `b0e368b`, 최초 run `29704717846` 149/150 PASS의 test expectation 오류, test-only correction `aea971d` 및 성공 run `29704796941` 150/150 PASS·32 suites(PostgreSQL 16.14 / Node.js 20.20.2, 신규 tests 25)를 additive 기록. 허용된 6-file implementation boundary와 Graph-only dependency를 확인하고 AC-014/AC-015는 RESOLVED / IN PROGRESS, 독립 리뷰 전 IMPLEMENTED·CLOSED·§9 PASS 미선언 상태로 유지 |
