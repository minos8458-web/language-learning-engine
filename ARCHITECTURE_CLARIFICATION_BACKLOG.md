...
**AC-016 implementation validation evidence(2026-07-20, additive)**: 기준 main은 `f8a5a38b4f32320dd5a5e1e696b97499d188bbf5`다. 검증 브랜치 `ac-016-learning-flow-validation-20260720`의 workflow-only commit `5f469884af991ad80183844beb991b865779326a`과 [baseline run 29746928577](https://github.com/minos8458-web/language-learning-engine/actions/runs/29746928577)에서 PostgreSQL 16.14 (Debian 16.14-1.pgdg13+1) / Node.js 20.20.2, 기존 150/150 PASS·32 suites를 확인했다. Implementation commit `d6d53203a52b784924737def7a9948f9f34bbdfa`은 허용된 4개 파일(`src/config/engineConfig.js`, 신규 `src/engines/learningFlowEngine.js`, 신규 `tests/learningFlowEngine.test.js`, 이 backlog)만 변경했다. [최초 implementation run 29747762577](https://github.com/minos8458-web/language-learning-engine/actions/runs/29747762577)은 175/183 PASS·39 suites로 실패했다. 원인은 `listNodesByLanguage` 결과에 존재하지 않는 `nodes[0].language`를 NEW_GRAMMAR active-count precheck에 전달한 production 결함이었고, 공개 이력을 재작성하지 않은 correction commit `860d9ddc84ebc8a805b65514c8f7918c35233277`에서 public `language` 인자를 helper로 전달하도록 수정했다. [최종 correction run 29747972286](https://github.com/minos8458-web/language-learning-engine/actions/runs/29747972286)은 신규 33개를 포함한 183/183 PASS·39 suites, 0 FAIL / 0 CANCELLED / 0 SKIPPED / 0 TODO였다. REVIEW·NEW_GRAMMAR·INTERLEAVING·CONVERSATION·IDLE priority chain, acknowledgement와 exact-key payload, prerequisite/admission, exhaustive Category·CONTRAST selection tuple, occurrence multiplicity, PRACTICING+ threshold, VI/EN 격리, 하위 오류 전파, config 소비, read-only/no-SQL/허용 dependency 경계를 실제 PostgreSQL 최소 합성 fixture로 검증했다. 변경하지 않은 보호 범위는 Tier A 및 API/Engine/Migration/Client/Validation 문서, DB schema·migration, `package.json`, 기존 하위 Engine과 기존 테스트다. 이 문단을 담는 evidence-only commit의 최종 SHA는 commit 생성 후 완료 보고에 함께 고정한다. AC-014/AC-015/AC-016은 모두 RESOLVED / Prerequisite Implementation IN PROGRESS이며 IMPLEMENTED·CLOSED 또는 Validation Level 3 §9 PASS를 선언하지 않는다.
...
| 1.26 | 2026-07-20 | AC-016 implementation validation evidence — baseline `5f46988` / run `29746928577` 150/150 PASS·32 suites, implementation `d6d5320` / 최초 run `29747762577` 175/183 PASS의 production language 전달 결함, correction `860d9dd` / 최종 run `29747972286` 183/183 PASS·39 suites(PostgreSQL 16.14 / Node.js 20.20.2, 신규 tests 33)를 additive 기록. 허용된 4-file implementation과 1-file correction, read-only/no-SQL/dependency 경계를 확인하고 AC-014/015/016은 RESOLVED / IN PROGRESS, IMPLEMENTED·CLOSED·§9 PASS 미선언 상태로 유지 |
