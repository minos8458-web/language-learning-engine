...
| AC-013 | Active-Node Admission Boundary | 🟡 Architecture Clarification **RESOLVED** / Prerequisite Implementation **IN PROGRESS** |
...
**상태**: 🟡 Architecture Clarification **RESOLVED** / Prerequisite Implementation **IN PROGRESS**(2026-07-18). 구현은 검증 브랜치 `ac-013-validation-20260718`에서만 진행 중이며 GitHub main에 병합되지 않았다. Actions 전체 검증·독립 코드리뷰·main merge 전에는 IMPLEMENTED·CLOSED·PASS로 전환하지 않는다.
...
**Implementation / Actions evidence(검증 브랜치 전용)**: Progress-side implementation commit은 `f61c30aaba0c6a1a806e3b19cfd43e30c2199338`이다. 최초 implementation Actions run `29616571586`은 97 tests 중 96 PASS / 1 FAIL을 반환했고, 실패 원인은 기존 `get_due_reviews` limit 테스트가 AC-013 gate 후에도 active STUDYING fixture 3개를 순차 admission하려 한 것이었다. 제품 로직을 완화하지 않고 조회 테스트의 의미를 유지한 PRACTICING fixture로 정정한 commit `a0a863fbdd4e776c7a450fcf906daad5e34a4d50` 후 Actions run `29616752159`가 **97/97 PASS, 21 suites, 0 FAIL, 0 CANCELLED**를 반환했다. 환경은 PostgreSQL 16.14 / Node.js 20.20.2다. 아직 main 미병합·독립 코드리뷰 전이므로 상태는 IN PROGRESS를 유지하며 IMPLEMENTED·CLOSED·PASS로 올리지 않는다.
...
| 1.17 | 2026-07-18 | AC-013 prerequisite implementation 검증 브랜치 착수 — 상태를 Architecture Clarification RESOLVED / Prerequisite Implementation IN PROGRESS로 전환. workflow-only commit `d06a9cd8`, baseline run `29616140994` 82/82 PASS(19 suites), implementation commit `f61c30aa`, 기존 due-review fixture 정정 commit `a0a863fb`, 최종 run `29616752159` 97/97 PASS(21 suites), PostgreSQL 16.14 / Node.js 20.20.2를 기록. main 미병합·독립 코드리뷰 전이며 IMPLEMENTED·CLOSED·PASS 선언 없음 |
