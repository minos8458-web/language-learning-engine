...
`submit_attempt` 처리 시 아래를 하나의 DB 트랜잭션으로 묶는다:
...
같은 트랜잭션 안에서, `error_category = TRANSFER`로 분류된 경우 `DOMAIN_LOGIC_BRIEF.md` §5.2가 찾은 대상 선행 노드 목록을 **`cascade_jobs` 테이블에 그대로 기록**한다(전용 메시지 큐 대신 단순 DB 테이블).
...
**처리 방식**: `cascade_jobs` 행 삽입은 핵심 트랜잭션(5.1)에 **함께 커밋**된다(같은 트랜잭션 — 이게 아웃박스 패턴의 핵심이다: "일이 일어났다는 사실"은 핵심 데이터와 항상 함께 저장되므로 유실되지 않는다). 실제 `progress.next_review_at` 갱신은 트랜잭션 밖에서, 별도의 워커(주기적 폴링, 예: 수 초 간격)가 `status='PENDING'` 행을 가져와 처리하고 `status='DONE'`으로 표시한다. 실패하면 `retry_count`를 올리고 `PENDING`으로 남겨 재시도한다.
